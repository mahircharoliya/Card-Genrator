...
export function FestivalCardGrid({
  cards,
  loading,
}: FestivalCardGridProps) {
  if (loading) {
    return (
  ...
  // Sort cards:
  // 1. Upcoming festivals first
  // 2. Nearest upcoming festival at top
  // 3. Past festivals at bottom
  // 4. Most recent past festival first
  const today = new Date();

  const sortedCards = [...cards].sort((a, b) => {
    const dateA = new Date(a.festivalDate);
    const dateB = new Date(b.festivalDate);
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
      {sortedCards.map((card, i) => {
        const festivalDate = new Date(card.festivalDate);
